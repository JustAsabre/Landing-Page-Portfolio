...
export default function PortfolioCard({
  ...
  return (
    <BlurFade delay={0.1} inView>
      <Card
        className="group overflow-hidden cursor-pointer hover-elevate active-elevate-2 transition-all duration-300 relative"
        onMouseEnter={() => setIsHovered(true)}
        onMouseLeave={() => setIsHovered(false)}
        onClick={onClick}
        data-testid={`card-portfolio-${title.toLowerCase().replace(/\s+/g, '-')}`}
      >
        {isHovered && <BorderBeam size={250} duration={8} delay={0} />}
